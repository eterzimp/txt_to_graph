        ...
        for (int i = 0; i < 2; i++) {
         switch (z1[i].cap) {
            ...
            case 1:
                ...
                break;
           /* case 2:
                legnames.push_back("38 pF");
                break;
            case 3:
                legnames.push_back("76 pF");
                break;
            case 4:
                legnames.push_back("106 pF");
                break;
            case 5:
                legnames.push_back("338 pF");
                break;
            case 6:
                legnames.push_back("368 pF");
                break;
            case 7:
                legnames.push_back("406 pF");
                break;
            case 8: 
                legnames.push_back("436 pF");
                break;   */
         }
        }
        break;
...
void PlotVectors(selection slct1, z read, vector<z> z1){ //Plot the filled vectors. This function takes as arguments:
    ...
    switch(slct1.arg_int[2]){
        ...
        case 3:
            ...
            break;
        case 4:
            mg->GetXaxis()->SetTitle("Temperature (grad celcius)");
            break;
    }
...
void plotdatauser() {
    ...
    while(1){
        cout << "Select variable to be constant('gain','peaktime','channel'): ";
        cin >> slct1.arg[0];
    ...
    while(1){ 
        if(slct1.arg[0]=="peaktime"){
            cout << "select a valid peaktime(0(:200),1(:100),2(:50),3(:25)): ";
            cin >> slct1.arg[4];
            break;
        } else if(slct1.arg[0]=="channel"){
            cout << "select a valid channel(0-63): ";
            cin >> slct1.arg[4];
            break; 
        } else if(slct1.arg[0]=="gain"){
            cout << "select a valid gain(0(:0.5), 1(:1.), 2(:3.), 3(:4.5), 4(:6.), 5(:9.), 6(:12.), 7(:16.)): ";
            cin >> slct1.arg[4];
            break;
        }
    }
    while (1) {
        cout << "Optional: select second variable to be constant (type q to skip): ";
        cin >> slct1.arg[5];
    ...
    if(slct1.arg[5]!="q"){
        if (slct1.arg[5] == "peaktime") {
            cout << "select a valid peaktime(0(:200),1(:100),2(:50),3(:25)): ";
            cin >> slct1.arg[6];
        }
        else if (slct1.arg[5] == "channel") {
            cout << "select a valid channel(0-63): ";
            cin >> slct1.arg[6];
        }
        else if (slct1.arg[5] == "gain") {
            cout << "select a valid gain(0(:0.5), 1(:1.), 2(:3.), 3(:4.5), 4(:6.), 5(:9.), 6(:12.), 7(:16.)): ";
            cin >> slct1.arg[6];
        }
    }
    ...
    while(1){    
        cout << "Select x axis('gain','peaktime','channel','capacitance','temperature'): ";
        cin >> slct1.arg[2];
        if(slct1.arg[2]!="peaktime" && slct1.arg[2]!="gain" && slct1.arg[2]!="channel" && slct1.arg[2]!="capacitance" && slct1.arg[2]!="temperature"){
            ...
            cout << "To have capacitance or temperature on x axis you must have two constant variables." << endl;
                    while(1){
                        cout << "Select a second constant:";
                        cin >> slct1.arg[5];
                        if(slct1.arg[5] != "peaktime" && slct1.arg[5] != "gain" && slct1.arg[5] != "channel") {
                            cout << "not a valid selection" << endl;
                            continue;
                        }else if (slct1.arg[5] == slct1.arg[0]) {
                            cout << slct1.arg[5] << " was already declared to be a constant" << endl;
    ...
    while(1){
        if(slct1.arg[5]=="q"){ //case where there is no second constant and no capacitance/temp on x axis 

        //-----------------------------------------------------------------------------------------
            cout << "Select z axis('gain','peaktime','channel','capacitance'): ";
            cin >> slct1.arg[3];
            ...
            if(slct1.arg[3]!="peaktime" && slct1.arg[3]!="gain" && slct1.arg[3]!="channel" && slct1.arg[3]!="capacitance"){
                ...
                continue;
            }else if(slct1.arg[0]==slct1.arg[3]){
                cout << slct1.arg[3] << " was declared to be a constant" << endl;
                continue;
            }else if(slct1.arg[3] == "channel") {
                    cout << "select up to 6 random channels: ";
                    for (int i = 0; i < 6; i++) {
                        cin >> slct1.chn_id[i];
                    }
                    break;
                    ...
                    while(1){
                        ...
                        if(slct1.arg[5] != "peaktime" && slct1.arg[5] != "gain" && slct1.arg[5] != "channel") {
                            ...
                            cout << slct1.arg[5] << " was already declared to be a constant" << endl;
                            continue;
                        }else if(slct1.arg[5] == slct1.arg[2]){
                            cout << slct1.arg[5] << " is already placed on x axis" << endl;
                            continue;
                        }else {
                            if (slct1.arg[5] == "peaktime") {
                                cout << "select a valid peaktime(0(:200),1(:100),2(:50),3(:25)): ";
...
//-------------------------------------------------------------------------------------
        ...
        }else if (slct1.arg[5]!="q" && slct1.arg[2]!="capacitance"){ //case there is constant and no capacitance on x axis
            while(1){
            cout << "Plot capacitance on z axis? ('yes' or 'no'): ";
            cin >> slct1.arg[3];
            if(slct1.arg[3]!="yes" && slct1.arg[3]!="no"){
                cout << "please answer with a 'yes' or 'no'" << endl;
                continue;
            }
            break;
            }
            if (slct1.arg[3]=="yes"){
                slct1.arg[3]="capacitance";
            }else{
 ...
 //---------------------------------------------------------------------
            ...
            while(1){
            cout << "Select z axis('gain','peaktime','channel'): ";
            cin >> slct1.arg[3];
            if(slct1.arg[3]!="peaktime" && slct1.arg[3]!="gain" && slct1.arg[3]!="channel"){
                cout << "not a valid selection" << endl;
                continue;
    ...
    while(1){
      if(slct1.arg[3]!="capacitance"){ //then x is cap or x is not cap. if x is cap, then z can be enc/rms as well. if x is not cap  
        cout << "Select y axis('enc','rms','enc_acqon','enc_acqoff','rms_acqon','rms_acqoff','pulse_ampl','temperature','step'): ";
        cin >> slct1.arg[1]; 
        ...
        slct1.arg[1]!="rms_acqoff" && slct1.arg[1] != "enc" && slct1.arg[1] != "rms"  && slct1.arg[1] != "temperature"  && slct1.arg[1] != "step"){
            cout << "not a valid selection" << endl;
            continue;
        }else if((slct1.arg[1] == "enc" || slct1.arg[1] == "rms") && slct1.arg[5]=="q") {
            cout << "previously chosen z axis will become a constant" << endl;
            slct1.arg[5] = slct1.arg[3];
            if(slct1.arg[5]=="peaktime"){
                cout << "select a valid constant peaktime(0(:200),1(:100),2(:50),3(:25)): ";
                cin >> slct1.arg[6];
               ...
               // slct1.chn_id[0];
                ...
                cout << "select a valid constant gain(0(:0.5), 1(:1.), 2(:3.), 3(:4.5), 4(:6.), 5(:9.), 6(:12.), 7(:16.)): ";
                cin >> slct1.arg[6];
            }
            break;
        }else{
      ...
      }else{ //else is either z is cap and x is not, or z is not cap and x is not cap , (both cannot be cap)
        cout << "Select y axis('enc_acqon','enc_acqoff','rms_acqon','rms_acqoff','pulse_ampl','temperature','step'): ";
        cin >> slct1.arg[1]; 
        if(slct1.arg[1]!="pulse_ampl" && slct1.arg[1]!="enc_acqon" && slct1.arg[1]!="enc_acqoff" && slct1.arg[1]!="rms_acqon" 
        && slct1.arg[1]!="rms_acqoff" && slct1.arg[1] != "temperature"  && slct1.arg[1] != "step"){
            cout << "not a valid selection" << endl;
            continue;  
        }
        break;
...
/*string createname(selection selection){
    ostringstream namestream;
    string name;
    if(selection.arg_int[0]==0){
        if (selection.arg_int[5] < 0) {
            namestream << selection.arg[0] << selection.arg[4] << "_" << selection.arg[1] << "_vs_" << selection.arg[2] << "_dif_" << selection.arg[3] << "s";
        }
        else if (selection.arg_int[5] >= 0 && (selection.arg_int[3]==3 || selection.arg_int[2]==3)) {
            namestream << selection.arg[0] << selection.arg[4] << "_" << selection.arg[1] << "_vs_" << selection.arg[2] << "_dif_" << selection.arg[3] << "s_" << selection.arg[5] << "_" << selection.arg[6];
        }
        else if (selection.arg_int[5] >= 0 && selection.arg_int[3]!=3  && selection.arg_int[2]!=3){
            namestream << selection.arg[0] << selection.arg[4] << "_" << selection.arg[1] << "_vs_" << selection.arg[2] << "_" << selection.arg[5] << "_" << selection.arg[6];
        }
        
    } else{
        if (selection.arg_int[5] < 0) {
            namestream << selection.arg[1] << "_vs_" << selection.arg[2] << "_dif_" << selection.arg[3] << "s" << "_" << selection.arg[0] << "_" << selection.arg[4];
        }
        else if (selection.arg_int[5] >= 0 && (selection.arg_int[3]==3 || selection.arg_int[2]==3)) {
            namestream << selection.arg[1] << "_vs_" << selection.arg[2] << "_dif_" << selection.arg[3] << "s" << "_" << selection.arg[0] << "_" << selection.arg[4] << "_" << selection.arg[5] << "_" << selection.arg[6];
        }
        else if (selection.arg_int[5] >= 0 && selection.arg_int[3]!=3 && selection.arg_int[2]!=3){
            namestream << selection.arg[1] << "_vs_" << selection.arg[2] << "_" << selection.arg[0] << "_" << selection.arg[4] << "_" << selection.arg[5] << "_" << selection.arg[6];
        }
    }
    name = namestream.str();
    return name;
